...
def exp(tmp_path, sample_data, pipeline):
    ...
    exp_obj.build(pipeline)
    exp_obj.exp(pipeline)
    return exp_obj
...
class TestToInferencer:
    def test_basic_creation(self, trained_trainer, pipeline):
        inf = trained_trainer.to_inferencer(pipeline)
    ...
    def test_not_trained_raises(self, tmp_path, exp, pipeline, sample_data):
        ...
        with pytest.raises(RuntimeError, match="not built"):
            trainer.to_inferencer(pipeline)

    def test_v_stored(self, trained_trainer, pipeline):
        inf = trained_trainer.to_inferencer(pipeline, v=[0])
        assert inf.v == [0]
...
class TestProcess:
    ...
    def test_v_parameter(self, tmp_path, exp, pipeline, sample_data):
        pipeline.set_grp('model_proba', role='head', processor=DecisionTreeClassifier,
                    ...
                    edges={'X': [('scaler', None)], 'y': [(None, ['target'])]},
                    params={'max_depth': 3, 'random_state': 42})
        pipeline.set_node('dt_proba', grp='model_proba')
        exp.build(pipeline)
        exp.exp(pipeline)
        trainer = _make_trainer(pipeline, 't_proba', sample_data, tmp_path / 'trainer_t_proba')
        trainer.select_head(pipeline, ['dt_proba'])
        trainer.train(pipeline)
        inf = trainer.to_inferencer(pipeline, v=slice(-1, None))
    ...
    def test_single_split(self, tmp_path, exp, pipeline, sample_data):
        ...
        trainer.select_head(pipeline, ['dt'])
        trainer.train(pipeline)
        inf = trainer.to_inferencer(pipeline)
        result = inf.process(sample_data)
        assert result.shape[0] == len(sample_data)
