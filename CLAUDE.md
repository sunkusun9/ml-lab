...
- **`remove_error_trials(experimenter=None)`**: 에러난 Trial을 전부 찾아 `remove_trial(name)`로 지운다. **검색만 `experimenter`로 좁혀지고 삭제는 안 좁혀진다** — `error_trials`/`pending_trials`처럼 이름을 주면 그 experimenter 이력에서만 에러를 찾지만, 찾은 이름은 언제나 `remove_trial`로 지워서 다른 experimenter의 결과까지 전부 걷어낸다(`remove_trial`이 이미 부분 삭제를 거부하므로 여기서 그 계약을 다시 쪼개지 않는다). 한 experimenter에서 에러났고 다른 experimenter에선 성공한 이름도 지운다 — "범위 안 어딘가에서 에러난 적 있나"를 묻는 것이지 "에러만 났나"가 아니다. 반환은 지운 이름 리스트(`TrialStore.get_hist` 정렬 순서, 알파벳순)
...
- **`get_collector(experimenter, name)` / `get_collect_hist(experimenter, collector_name=, node_name=, status=, pipeline_version=)` / `build(experimenter, ..., os_log=True)` / `exp(experimenter, trials, ..., os_log=True)`**: 같은 자리의 대행 메소드 셋 — 각각 `Experimenter.collectors.get_collector`/`Experimenter.collectors.hist.get_hist`/`Experimenter.build`/`Experimenter.exp`에 인자 그대로 위임. `set_collector`처럼 소유권은 없고 `project.experimenters[experimenter].X(...)`(`get_collect_hist`는 3단: `.collectors.hist.get_hist(...)`)의 반복 접근을 줄여주는 편의일 뿐이라, Experimenter를 여러 개 관리할 때 이름 하나로 조회/빌드/실행을 바로 걸 수 있다. 게이트를 추가하는 게 아니므로 `Experimenter.exp`/`build`가 이미 언제나 호출 가능하다는 성질(상태 게이트 없음)은 그대로 유지
  - **`get_collect_hist`는 `collect_errors`와 다르다** — `collect_errors`는 여러 Experimenter를 팬아웃해 `experimenter` 키를 붙여 합치지만(`status='error'`로 고정), `get_collect_hist`는 이름 하나로 좁혀 `CollectHist.get_hist`가 내놓는 걸 그대로 돌려준다(필터 전부 그대로 통과, `status` 고정 없음). 여러 Experimenter를 가로지르는 질문이 아니라 하나의 이력을 있는 그대로 보고 싶을 때 씀
  - **`os_log`는 `build`/`exp`에만 있다** (`get_collector`엔 없음) — 실제로 native 라이브러리(TensorFlow/LightGBM/CatBoost/cuDNN 등)가 fd 1/2에 직접 뭔가를 쓸 수 있는 자리는 이 둘뿐이라서. 기본 `True`: `with e.os_log(): e.build(...)`/`e.exp(...)`로 감싸 그 잡음을 콘솔 대신 `{exp_path}/__worker_logs/`로 보낸다. `False`를 주면 감싸지 않고 예전처럼 직접 호출 — 호출자가 이미 `with e.os_log():` 블록 안에서 여러 호출을 묶어 쓰는 경우(`open_os_log`는 이미 열려 있으면 `RuntimeError`) 이 자리에서 쓸 옵션. 대행이 게이트를 만드는 게 아니라 `open_os_log`가 이미 갖고 있던 제약을 그대로 물려받는 것뿐
...
- `get_by_name(name)`/`list_trials(name=, processor=, method=, pipeline_version=, src_trial=, tag=, desc=)`: **`Trial` 객체**를 반환(`PredictorStore`와 같음). `exp()`가 이름으로 받아 여기서 정의를 꺼내 실행하므로, 넣은 것과 같은 것이 나와야 함
  - `list_trials`의 필터는 전부 SQL `WHERE`(AND 결합, `None`이면 스킵) — `get_hist`/`remove_hist`와 같은 동적 WHERE 모양. `name`/`processor`/`method`/`pipeline_version`/`src_trial`은 정확 일치, `desc`는 `LIKE '%...%'`(자유 서술문이라 부분일치), `tag`는 SQLite JSON1의 `json_each`로 **리스트 소속 여부**를 봄 — `tag` 컬럼은 JSON 배열 TEXT라, 저장된 문자열에 직접 부분일치를 걸면 `'b'`가 `'ab'`의 JSON 인코딩 안에서 잘못 걸릴 수 있어 그렇게 안 함. Pipeline 쪽 `get_node_names(query)`(None/list/정규식) 관례와는 **의도적으로 다르다** — 여긴 SQL 컬럼 값 매칭이 자연스러운 자리라서
...
- `record(trial_name, experimenter, outer_idx, inner_idx, pipeline_version, status, recorded_at=)`, `get_hist(trial_name=, experimenter=, pipeline_version=, status=)`, `get_status(...)`, `remove_hist(...)`
  - `recorded_at`은 이 행이 쓰인 시점(안 주면 `_common.utc_now()`, `NodeStore.node_hist`/`CollectHist.collect_date`와 같은 포맷) — `info['started_at']`(`_process()`가 실행을 시작한 시점)과는 다른 질문에 답한다
...
- 아티팩트: `{path}/{outer_idx}/{inner_idx}/{node_name}/`
  - `obj.pkl` — processor 객체, `result.pkl` — fit_transform/fit_predict 출력. **info.pkl 없음** — status/definition/started_at/fit_time/edges/train_shape/error는 전부 history에
...
- **history**: SQLite `node_hist(node_name, outer_idx, inner_idx, pipeline_version, status, recorded_at, info)` — PK는 `(node_name, outer_idx, inner_idx)`(Experimenter/Trainer 이름 컬럼 없음 — store 자체가 이미 거기에 스코프돼 있음)
  - `record(name, outer_idx, inner_idx, pipeline_version=, status=, recorded_at=, info=)` — `info`는 `status` 제외 나머지 전부(`build_id`, `definition`, `started_at`, `fit_time`, `edges`, `train_shape`, `warnings`, 실패 시 `error`) JSON 인코딩. `NodeInfoTracker`(`_tracker.py`)가 기록
  - **두 시점이 다른 걸 답한다**: `info['started_at']`는 `_process()`(`_executor.py`)가 실제로 돌기 시작한 순간, `recorded_at`은 그 결과가 이 테이블에 쓰인 순간 — 성공/실패 둘 다 `_process()`가 `started_at`을 채워 넣고, `record()`가 인자 없으면 `recorded_at`을 그때그때 새로 찍는다(`CollectHist.collect_date`와 같은 방식·같은 포맷, `_common.utc_now()` 공유). 멀티워커에선 워커가 처리를 시작한 시점과 부모가 그 결과를 받아 적는 시점이 갈릴 수 있어 둘을 구분해 남긴다
...
- **_common.py**: Experimenter/Trainer 공용 — `require_built_pipeline`, `require_published_pipeline`(양쪽 공용 채택 게이트 — draft만 거부), `resolve_common_status`, `save_pipeline(path, pipeline)`/`load_pipeline(path)`(`{path}/pipeline.pkl`, 없으면 `None`), `error_payload`, `utc_now()`(ISO-8601 UTC, 초 단위 — `CollectHist.collect_date`/`node_hist`·`experiment_hist`의 `recorded_at`이 공유하는 시점 포맷)
